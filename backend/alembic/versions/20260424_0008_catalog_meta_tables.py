...
def upgrade() -> None:
    ...
    # Paleta unificada: fuel, sector, emission, pwr (tecnologías eléctricas), family.
    op.create_table(
        "catalog_meta_color_palette",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("key", sa.String(128), nullable=False),
        sa.Column("color_hex", sa.String(9), nullable=False),  # "#RRGGBB" o "#RRGGBBAA"
    ...
    # ─── catalog_meta_label ─────────────────────────────────────────────────
    # Código → nombre display (reemplaza DISPLAY_NAMES y NOMBRES_COMBUSTIBLES).
    op.create_table(
        "catalog_meta_label",
        sa.Column("id", sa.Integer(), primary_key=True),
        ...
        sa.Column("category", sa.String(64), nullable=True),  # "fuel" | "tech" | "emission" | "sector" | "var_capacidad" | ...
        sa.Column("sort_order", sa.Integer(), nullable=False, server_default="0"),
        *_audit_columns(),
    ...
    # Familia → prefijos tech (reemplaza FAMILIAS_TEC).
    op.create_table(
        "catalog_meta_tech_family",
        sa.Column("id", sa.Integer(), primary_key=True),
    ...
    # Módulos del ChartSelector (nivel 1): electrico, demanda, ...
    op.create_table(
        "catalog_meta_chart_module",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("code", sa.String(64), nullable=False),
        sa.Column("label", sa.String(128), nullable=False),
        sa.Column("icon", sa.String(16), nullable=True),  # emoji
        sa.Column("sort_order", sa.Integer(), nullable=False, server_default="0"),
    ...
    # Submódulos bajo un módulo (nivel 2): residencial, industrial, ...
    op.create_table(
        "catalog_meta_chart_submodule",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column(
            "module_id",
            sa.Integer(),
            sa.ForeignKey("osemosys.catalog_meta_chart_module.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column("code", sa.String(64), nullable=False),
        sa.Column("label", sa.String(128), nullable=False),
    ...
    # Configuración completa de cada gráfica (reemplaza CONFIGS).
    op.create_table(
        "catalog_meta_chart_config",
        sa.Column("id", sa.Integer(), primary_key=True),
        ...
        sa.Column(
            "module_id",
            sa.Integer(),
            sa.ForeignKey("osemosys.catalog_meta_chart_module.id", ondelete="RESTRICT"),
            nullable=False,
        ),
        sa.Column(
            "submodule_id",
            sa.Integer(),
            sa.ForeignKey("osemosys.catalog_meta_chart_submodule.id", ondelete="SET NULL"),
            nullable=True,
        ),
        ...
        # Filtro: kind enum + params JSON (evita serializar callables).
        ...
        # Flags + texto
        sa.Column("flags_json", postgresql.JSONB(), nullable=True),  # {es_capacidad, es_porcentaje, es_emision, has_loc, soporta_pareto}
        sa.Column("msg_sin_datos", sa.String(512), nullable=True),
        # Data Explorer integration (Fase 2)
    ...
    # Sub-filtros por gráfica (CARRETERA, AVI, CKN, BOI, ...).
    op.create_table(
        "catalog_meta_chart_subfilter",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column(
            "chart_id",
            sa.Integer(),
            sa.ForeignKey("osemosys.catalog_meta_chart_config.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column("group_label", sa.String(64), nullable=True),  # "Modo", "Uso", "Combustible"
        sa.Column("code", sa.String(64), nullable=False),
    ...
    # Agrupaciones combinables de sub-filtros ("Transporte liviano" = [LDV, TAX, ...]).
    op.create_table(
        "catalog_meta_chart_subfilter_group",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column(
        ...
        sa.Column("subfilter_codes_json", postgresql.JSONB(), nullable=False),  # ["CARRETERA","LDV",...]
        sa.Column("sort_order", sa.Integer(), nullable=False, server_default="0"),
        *_audit_columns(),
        schema=SCHEMA,
    )
    ...
    # Unidad base + conversiones por variable.
    op.create_table(
        "catalog_meta_variable_unit",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("variable_name", sa.String(128), nullable=False),
        sa.Column("unit_base", sa.String(32), nullable=False),
        sa.Column("display_units_json", postgresql.JSONB(), nullable=True),  # [{code:"GW",label:"GW",factor:31.536,...}]
        *_audit_columns(),
    ...
    # ─── catalog_meta_audit ─────────────────────────────────────────────────
    # Log global de cambios sobre cualquier tabla catalog_meta_*.
    op.create_table(
        "catalog_meta_audit",
        sa.Column("id", sa.Integer(), primary_key=True),
